...
% Generates N data points in given dimension
    ...
    while(d >0)
        ...
        v= normc(v); %col vector (v is a random point in currentDim + 1)
        v
        e = zeros(currentDim+1,1); %col vector
        e(currentDim+1) = 1; % e is [0 0. . .0 1]'
        
        
        rotationMat = rot(v, e); %matrix of dim [currentDim+1 x currentDim+1]
                                   %rotates v to e
        ...
        d = d - 1;
        X = rotateAll(X,rotationMat); %rotate the points via rotationMatrix
