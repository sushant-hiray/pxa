...
load('../../data/primaindiandiabetes.mat');
pimaindiandiabetes = pimaindiandiabetes';
rng(0);
PGS =1;
options.KernelType='Gaussian';
[Mapping,BkGm,R_kernel,Res] = kernel_pnsMain(pimaindiandiabetes,1,PGS,options);
[ eigvec, eig_val ] = kpca_main(pimaindiandiabetes,options);
R_kpca_gauss = 100*eig_val/sum(eig_val);


% options.KernelType='NPolynomial';
% options.degree=3;
% [Mapping,BkGm,R_kernel_np3,Res] = kernel_pnsMain(pimaindiandiabetes,1,PGS,options);
% [ eigvec, eig_val ] = kpca_main(pimaindiandiabetes,options);
...
% options.KernelType='NPolynomial';
% options.degree=5;
% [Mapping,BkGm,R_kernel_np5,Res] = kernel_pnsMain(pimaindiandiabetes,1,PGS,options);
% [ eigvec, eig_val ] = kpca_main(pimaindiandiabetes,options);
