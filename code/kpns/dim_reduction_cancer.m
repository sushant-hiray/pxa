...
[Mapping,BkGm,R_kernel,Res] = kernel_pnsMain(cancer,1,PGS,options);
[ eigvec, eig_val ] = kpca_main(cancer,options);
R_kpca_gauss = 100*eig_val/sum(eig_val);


options.KernelType='NPolynomial';
options.degree=3;
[Mapping,BkGm,R_kernel_np3,Res] = kernel_pnsMain(cancer,1,PGS,options);
[ eigvec, eig_val ] = kpca_main(cancer,options);
...
options.KernelType='NPolynomial';
options.degree=5;
[Mapping,BkGm,R_kernel_np5,Res] = kernel_pnsMain(cancer,1,PGS,options);
[ eigvec, eig_val ] = kpca_main(cancer,options);
